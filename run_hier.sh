...
echo "==================== arguments ==================== ";
echo "model=$MODEL, dataset=$DATASET, hosts=$HOSTS, num_workers=$NWORKER, format=$FORMAT";
...
echo -e "\n=================== mpirun command: ================== "
MPICMD="mpirun -np $NWORKER $HVD_PREFIX -x MXNET_SAFE_ACCUMULATION=1 $CMD"
echo -e "$MPICMD \n =====================================================\n"

mpirun -np $NWORKER --mca plm_rsh_agent 'ssh -q -o StrictHostKeyChecking=no' \
